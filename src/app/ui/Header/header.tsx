...
const Header: React.FC<HeaderProps> = ({ categories }) => { // Принимаем categories как пропс
            ...
            <div className={styles.container}>
                ...
                <div className={styles.header_left}>
                    {categories.map((category) => (
                        <div
                            key={category.id}
                            className={`${styles.link} ${(selectCategory === category.id.toString() || activeCategory == category.id.toString()) ? styles.link_active : ''}`}
                        ...
                        >
                            {(showSubcategories === category.id.toString() || selectCategory === category.id.toString() || category.id.toString() === activeCategory)
                                ? <Link href={`/${ category.typeOfServiceId === 1 ? "raboty/" + category.url : "materialy/" + category.url } `}>
                                    <div className={styles.header_left_img}>
                                        <Image width={80} height={80} src={`${process.env.NEXT_PUBLIC_API_URL}/${сhangeExtantion(category.img)}`} alt={category.name} />
                                    </div>
                                </Link>
                                : <Link href={`/${ category.typeOfServiceId === 1 ? "raboty/" + category.url : "materialy/" + category.url } `}>
                                    <div>
                                        <Image width={50} height={50} src={`${process.env.NEXT_PUBLIC_API_URL}/${category.img}`} alt={category.name} />
                                    </div>
                            ...
                            {showSubcategories === category.id.toString() &&
                                <div className={styles.subcategories}>
                                    {category.sub.map(subcategory => (
                                        <Link key={subcategory.idSub} href={`/${category.typeOfServiceId === 1 ? "raboty/" + category.url + "/" + subcategory.url : "materialy/" + category.url + "/" + subcategory.url}`} className={styles.subcategory}>
                                            {subcategory.title}
                                        </Link>
                                    ))}
                                </div>
